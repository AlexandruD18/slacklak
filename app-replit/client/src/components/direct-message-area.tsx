...
import type { User, DirectMessageWithUsers } from "@shared/schema";

interface DirectMessageAreaProps {
  user: User;
}

export function DirectMessageArea({ user: otherUser }: DirectMessageAreaProps) {
  ...
  const sendMutation = useMutation({
    mutationFn: async (content: string) => {
      const response = await apiRequest("POST", `/api/dm/${otherUser.id}`, { content });
      return response;
    },
    onSuccess: () => {
      setMessage("");
  ...
  useEffect(() => {
    const unsubscribeDM = subscribe("dm:new", (payload) => {
      if (
    ...
    const unsubscribeTypingStart = subscribe("dm:typing:start", (payload) => {
      if (payload.senderId === otherUser.id && payload.receiverId === user?.id) {
        setTypingUser(otherUser.name);
      }
    });

    const unsubscribeTypingStop = subscribe("dm:typing:stop", (payload) => {
      if (payload.senderId === otherUser.id) {
        setTypingUser(null);
      }
    });
  ...
  }, [otherUser.id, otherUser.name, subscribe, user?.id]);
  ...
  const handleTyping = () => {
    if (!isTyping) {
      setIsTyping(true);
      sendSocketMessage("dm:typing:start", { receiverId: otherUser.id });
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      setIsTyping(false);
      sendSocketMessage("dm:typing:stop", { receiverId: otherUser.id });
    }, 2000);
  };
  ...
  const groupMessagesByDate = (messages: DirectMessageWithUsers[]) => {
    const groups: { date: string; messages: DirectMessageWithUsers[] }[] = [];
    let currentDate = "";
              ...
              <div key={group.date}>
                ...
                <div className="space-y-1">
                  {group.messages.map((msg, index) => {
                    ...
                    const sender = msg.senderId === user?.id ? user : otherUser;
